...
### O build de produção nunca tinha sido testado, e nem o smoke test que foi escrito para testá-lo

Preparando a imagem Docker do hub (Fase 3, item de deploy), `npm run start:prod` — o comando que a imagem realmente usa — se revelou quebrado desde a Fase 1: `node dist/main` não encontrava o arquivo, porque sem `rootDir` explícito e com `prisma/seed.ts` também sendo compilado, o TypeScript inferia a raiz comum como a raiz do projeto e gerava `dist/src/main.js`, não `dist/main.js`. Ninguém tinha notado porque toda verificação desta sessão inteira usou `nest start` (modo dev via ts-node), nunca o binário compilado. Corrigido fixando `rootDir: "src"` e excluindo `prisma/` do build.

Corrigido isso, a imagem buildava mas o container morria ao conectar no MySQL: o Alpine não tem `libssl` por padrão, e o engine binary do Prisma carregava a variante errada — um erro que só aparece em runtime, depois que `docker build` já terminou sem reclamar. Corrigido com `apk add openssl` nos dois estágios da imagem.

Para não depender de lembrar de testar a imagem manualmente da próxima vez, foi adicionado um job de CI que builda a imagem e confirma que ela responde em `/health`. Esse job, por sua vez, teve dois bugs próprios, cada um só aparecendo ao rodar de verdade no runner do GitHub Actions (não é possível simular `bash -e` de um runner Linux com precisão só lendo o script): primeiro, `curl -w "%{http_code}" ... || echo "000"` produzia `"000000"` (o próprio curl já imprime `"000"` numa conexão recusada, e o `|| echo` rodava *também*, concatenando), e por coincidência `"000000" != "000"` fazia o teste "passar" na primeira tentativa, sem o container ter subido de verdade. Corrigido comparando com os códigos válidos esperados (`"200"` ou `"503"`) em vez de "qualquer coisa diferente de 000" — e essa correção *quebrou* o job, porque exposto o segundo bug: os passos do GitHub Actions rodam com `bash -e`, e o exit code 7 do curl (conexão recusada, esperado nas primeiras tentativas) abortava o script inteiro antes do laço de espera rodar uma única vez. Corrigido com `|| true` isolado no `curl`, sem reintroduzir o problema da concatenação.

Quatro bugs em sequência, todos no código que existe para verificar se o resto funciona, e nenhum detectável sem executar de verdade contra um ambiente real — o padrão se repete mesmo quando o que está sendo escrito é a própria rede de proteção.
...
Os erros que valem a pena lembrar têm o mesmo formato: aconteceram exatamente onde o sistema precisa ser confiável sob concorrência, sob um caminho de dado que não é o feliz, ou num ambiente de execução que difere do ambiente de desenvolvimento — e nenhum apareceu em um teste unitário. Cada um só foi encontrado rodando alguma coisa de verdade (a aplicação, o CI, o próprio smoke test) e comparando o resultado com o esperado, nunca relendo o código e concluindo que estava correto. A regra prática que fica: a IA (e quem revisa o trabalho dela) não deveria confiar que um padrão foi seguido só porque está documentado em algum ADR do repositório, nem que um caminho funciona só porque outro caminho parecido já foi testado — vale a pena executar cada caminho novo pelo menos uma vez, incluindo os que existem só para verificar os outros.
